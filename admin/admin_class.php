...
Class Action {
	...
	function save_forum(){
		...
		$data .= ", description = '".htmlentities(str_replace("'","&#x2019;",$description))."' ";
		// $data .= ", blogimg = '". $img . "'";
		// if($_FILES['blogimg']['tmp_name'] != ''){
		// 	$_FILES['blogimg']['name'] = str_replace(array("(",")"," "), '', $_FILES['banner']['name']);
		// 	$fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['banner']['name'];
		// 	$move = move_uploaded_file($_FILES['banner']['tmp_name'],'assets/uploads/'. $fname);
		// $data .= ", blogimg = '$fname' ";
	...
	function save_post(){
		extract($_POST);
		$data = " title = '$title' ";
		$data .= ", description = '".htmlentities(str_replace("'","&#x2019;",$content))."' ";
		if($_FILES['banner']['tmp_name'] != ''){
						$_FILES['banner']['name'] = str_replace(array("(",")"," "), '', $_FILES['banner']['name']);
						$fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['banner']['name'];
						$move = move_uploaded_file($_FILES['banner']['tmp_name'],'assets/uploads/'. $fname);
